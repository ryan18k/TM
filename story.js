...
// Indications scéniques : guillemets français + italique via classe CSS
...
async function runGame(){
  ...
  do {
    mg2 = await runMG_dosage({
      ...
      failMessage:'La perfusion a été interrompue. Une main ferme est indispensable.',
    });
  } while(mg2===false);
  ...
  setProgress(100);
  await fadeOut(800); stageHideAll(); await sleep(500);
...
async function startGame(){
  ...
  window._actx=AC;
  initMusic();
  // Fond + musique dès l'écran titre
  setBgScene('hospital');
  playMusicAmbiance();
